...
@Service
public class M7000DatalogParse implements DatalogParse {
    @Override
    public HashMap<String, String> parseName(String fileName) {
        ...
        try {
            ...
            for (int j = 0; j < NameTokens.length; j++) {
                if (NameTokens[j].contains("TTM") || NameTokens[j].contains("TTT") || NameTokens[j].contains("TTV")) {
                    if (flag) {
                        TesterID = j;
                        flag = false;
                    }
            ...
            if (fileName.endsWith("Summary.csv.zip"))
            {
                CP=NameTokens[Length - 5];
            }
            if (fileName.endsWith("datalog_1.csv.zip"))
            {
                CP=NameTokens[Length - 6];
            }
