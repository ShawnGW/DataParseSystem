...
@Service
public class ChromaDatalogParse implements DatalogParse {
    @Override
    public HashMap<String, String> parseName(String fileName) {
        ...
        try {
            ...
            for (int j = 0; j < NameTokens.length; j++) {
                if (NameTokens[j].contains("TTH") || NameTokens[j].contains("TTT") || NameTokens[j].contains("TTV")) {
                    if (flag) {
                        TesterID = j;
                        flag = false;
                    }
