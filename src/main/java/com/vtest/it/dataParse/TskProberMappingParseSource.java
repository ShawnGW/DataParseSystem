...
public class TskProberMappingParseSource {
	...
	public void setGetRandomNumber(GetRandomNumber getRandomNumber) {
		...
		public  HashMap<String, String> Get(File file,int gpibBin,HashMap<Integer, Integer> Bin_summary_Map,HashMap<String, String> DieMap,HashMap<String, String> skipAndMarkDieMap) throws IOException, NoSuchFieldException
		{
			String regex="[0-9]{1}";
			...
			String cp=null;
			Matcher matcher=pattern.matcher(Device.substring(Device.length()-1, Device.length()));
			if (matcher.find()) {
				cp=Device.substring(Device.length()-1, Device.length());
			}else {
				cp=Device.substring(3,4);
			}
			Matcher matcherAgain=pattern.matcher(cp);
			if (!matcherAgain.find()) {
					throw new NoSuchFieldException();
			}
			resultMap.put("Wafer ID", Wafer_ID.trim());
			resultMap.put("Operator", OP.trim());
			resultMap.put("CP Process", "CP"+cp);
